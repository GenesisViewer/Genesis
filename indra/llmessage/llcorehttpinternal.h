...
//
// - Implement policy classes. Structure is mostly there just didn't need it
//   for the first consumer (classes are there but more advanced features, like
//   borrowing, aren't there yet).
// - Consider Removing 'priority' from the request interface. Its use in an
//   always active class can lead to starvation of low-priority requests.
//   Requires coodination of priority values across all components that share a
//   class. Changing priority across threads is slightly expensive (relative to
//   gain) and hasn't been completely implemented. And the major user of
//   priority, texture fetches, may not really need it.
...
// - Global optimizations: 'borrowing' connections from other classes, HTTP
//   pipelining.
// - Dynamic/control system stuff: detect problems and self-adjust. This won't
//   help in the face of the router problems we've looked at, however. Detect
//   starvation due to UDP activity and provide feedback to it.
// - Change the transfer timeout scheme.  We're less interested in absolute
...
// - LLTextureFetch still needs a major refactor. The use of LLQueuedThread
//   makes it hard to inspect workers and do the resource waiting we're now
//   doing. Rebuild along simpler lines some of which are suggested in new
//   commentary at the top of the main source file.
// - Adapt texture cache, image decode and other image consumers to the
//   BufferArray model to reduce data copying. Alternatively, adapt this
//   library to something else.
// --------------------------------------------------------------------

// If '1', internal ready queues will not order ready requests by priority,
// instead it's first-come-first-served. Reprioritization requests have the
// side-effect of then putting the modified request at the back of the ready
// queue.
