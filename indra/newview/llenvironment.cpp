...
inline LLSettingsDay::track_bound_t
...
// 	// LLCoros::getInstance()->launch("LLEnvironmentRequest::environmentRequestCoro",
...
public:
	...
	:	LLSettingsBlenderTimeDelta(target, LLSettingsBase::ptr_t(),
								   LLSettingsBase::ptr_t(), LLSettingsBase::Seconds((F64)1.0)),
		mDay(day),
...
protected:
	...
	void initializeTarget(F64 time)
	{
		...
		setTimeSpent(LLSettingsBase::Seconds(blendf * (F64)mBlendSpan));
		setBlendFactor(blendf);
	}

	inline F64 getAdjustedNow() const
	{
		return LLTimer::getTotalSeconds() + mCycleOffset;
	}
...
:	LLEnvironment::DayInstance(env)
{
    ...
    //genesis comment
    //something to do when the user change parcel                                                       
...
{
	// LLCoros::instance().launch("LLDayInjection::testExperiencesOnParcel",
	// 			  [this, parcel_id]()
	// 			  {
	...
	if (!mInjection->isOverriddenSky())
	{
		...
		mBlenderSky =
			std::make_shared<LLSettingsBlenderTimeDelta>(mSky, mStartSky,
														 ...
														 LLSettingsBase::Seconds( mTransitionTime));
...
void LLEnvironment::initClass()
{
    ...
    //genesis comment
    //something to do when user change region or parcel
	/**mRegionChangedConnection =
		gAgent.addRegionChangedCB(boost::bind(&LLEnvironment::onRegionChange,
											  this));
	mParcelChangedConnection =
		LLViewerParcelMgr::getInstance()->addAgentParcelChangedCB(boost::bind(&LLEnvironment::onParcelChange,
												 this));
...
void LLEnvironment::cleanupClass()
{
    ...
    // LLEventPump& pump = LLEventPumps::instance().obtain(PUMP_EXPERIENCE);
...
bool LLEnvironment::canAgentUpdateParcelEnvironment(LLParcel* parcel)
{
	if (!parcel)
	{
		parcel = LLViewerParcelMgr::getInstance()->getSelectedOrAgentParcel();
	}
...
void LLEnvironment::onRegionChange()
{
    ...
    // regionp->setCapabilitiesReceivedCallback(boost::bind(&LLEnvironment::requestRegion,
	// 									   env_apply_fn()));                                    
}
									...
									
{
	if (!settings || status)
	{
		...
		args["DESC"] = LLTrans::getString("asset_id") + ": " +
					   asset_id.asString();
        LLNotifications::getInstance()->add("FailedToFindSettings", args);
...
void LLEnvironment::update(const LLViewerCamera* cam)
{
	...
	if (!can_use_shaders)
	{
		...
		gDeferredWLSunProgram.mUniformsDirty = true;
		gDeferredWLMoonProgram.mUniformsDirty = true;
	}
    LLViewerShaderMgr *mViewerShaderMgrp= LLViewerShaderMgr::instance();
	for (LLViewerShaderMgr::shader_iter it = mViewerShaderMgrp->beginShaders(),
										end = mViewerShaderMgrp->endShaders();
		 it != end; ++it)
	{
												 ...
												 const LLSettingsBase::ptr_t& settings)
{
	...
	{
		LL_INFOS() << "clearing uniform " << i << LL_ENDL;
		uniforms[i].clear();
	}
	...
	if (has_legacy_haze)
	{
		haze_it = settings->mSettings.map().find(LLSettingsSky::SETTING_LEGACY_HAZE);
	...
	for (const auto& it : params)
	{
		...
		if (has_legacy_haze)
		{
			param_it = haze_it->second.map().find(key);
		...
		{
			param_it = settings->mSettings.map().find(key);
		...
		switch (setting_type)
		{
			case LLSD::TypeInteger:
				LL_INFOS() << "updateGLVariablesForSettings " << it.second.getShaderKey()<< value.asInteger() << LL_ENDL;
				shader->uniform1i(it.second.getShaderKey(), value.asInteger());
				break;

			case LLSD::TypeReal:
				LL_INFOS() << "updateGLVariablesForSettings " << it.second.getShaderKey()<< value.asReal() << LL_ENDL;
				shader->uniform1f(it.second.getShaderKey(), value.asReal());
				break;

			case LLSD::TypeBoolean:
				LL_INFOS() << "updateGLVariablesForSettings " << it.second.getShaderKey()<< value.asBoolean() << LL_ENDL;
				shader->uniform1i(it.second.getShaderKey(),
								  value.asBoolean() ? 1 : 0);
				break;

			case LLSD::TypeArray:
				LL_INFOS() << "updateGLVariablesForSettings " << it.second.getShaderKey()<< LLVector4(value) << LL_ENDL;
	...
	gEnvironment.mCurrentEnvironment.updateShaderUniforms(shader);
}
...
{
	LL_INFOS() << "LLEnvironment::updateSettingsUniforms()" << LL_ENDL;
	if (mCurrentEnvironment->getWater())
	{
		LL_INFOS() << "LLEnvironment::updateSettingsUniforms() water" << LL_ENDL;
		updateGLVariablesForSettings(mWaterUniforms,
									 mCurrentEnvironment->getWater());
	}
	if (mCurrentEnvironment->getSky())
	{
		LL_INFOS() << "LLEnvironment::updateSettingsUniforms() sky" << LL_ENDL;
...
// 	else
// 	{
// 		LLNotifications::getInstance()->add("NoEnvironmentSettings");
// 	}
// }
...
// 			if (!cb)
// 			{
// 				F64 transition = gAgent.getTeleportState() != LLAgent::TELEPORT_NONE ||
// 								 LLViewerParcelMgr::getInstance()->waitingForParcelInfo() ?
											  ...
											  altitudes_vect_t altitudes)
{
	if (status)
	{
		LLNotifications::getInstance()->add("FailedToLoadSettingsApply");
		return;
	}
...
// {
	...
	// 	LLNotifications::getInstance()->add("WLRegionApplyFail", notify);
	// 	return;
	// }
...
bool LLEnvironment::DayInstance::applyTimeDelta(F64 delta)
{
	...
	if (mBlenderSky)
	{
		changed = mBlenderSky->applyTimeDelta(LLSettingsBase::Seconds(delta));
	}
	if (mBlenderWater)
	{
		changed |= mBlenderWater->applyTimeDelta(LLSettingsBase::Seconds(delta));
	}
	return changed;
}
...
F32 LLEnvironment::DayInstance::getProgress() const
{
	...
	F64 now = LLTimer::getTotalSeconds() + (F64)mDayOffset;
