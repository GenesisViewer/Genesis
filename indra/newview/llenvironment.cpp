...
void LLEnvironment::initOnLogin()
{
    ...
    //LLViewerParcelMgr::addAgentParcelChangedCallback([this]() { onParcelChange(); });
...
//     }
    ...
    if (!cur_region || cur_region->getRegionID().isNull())
    {
        return;
    }
...
{
    LLSettingsBase::Seconds transitionTime(static_cast<F64>(0.0f));
...
{
    LL_INFOS() << "LLEnvironment::updateEnvironment start" << LL_ENDL;
    DayInstance::ptr_t pinstance = getSelectedEnvironmentInstance();
    LLSD legacySettings;
    if (pinstance->getSky()) {
        legacySettings = LLSettingsVOSky::convertToLegacy(pinstance->getSky(),TRUE);
        LLEnvManagerNew::getInstance()->useSkyParams(legacySettings);
    }
    if (pinstance->getWater()) {
        legacySettings = LLSettingsVOWater::convertToLegacy(pinstance->getWater());
        LLEnvManagerNew::getInstance()->useWaterParams(legacySettings);
    }
    if (pinstance->getDayCycle()) {
        legacySettings = LLSettingsVODay::convertToLegacy(pinstance->getDayCycle());
        LLEnvManagerNew::getInstance()->useDayCycleParams(legacySettings,LLEnvKey::SCOPE_LOCAL);
    ...
    if(targetWater && targetWater->getNormalMapID()==LLUUID("8dcd4a48-2d37-4909-9f78-f7a9eb4ef903"))
    {
    ...
    LL_INFOS() << "LLEnvironment::updateEnvironment end" << LL_ENDL;
}
...
//-------------------------------------------------------------------------
void LLEnvironment::update(LLViewerCamera * cam)
{
    ...
    // //updateCloudScroll();
    // LLWLParamManager::getInstance()->updateCloudScrolling();
    ...
    //     legacySettings = LLSettingsVOSky::convertToLegacy(mCurrentEnvironment->getSky(),TRUE);
    //     LLEnvManagerNew::getInstance()->useSkyParams(legacySettings);
    // }
    // if (mCurrentEnvironment->getWater()) {
    //     legacySettings = LLSettingsVOWater::convertToLegacy(mCurrentEnvironment->getWater());
    //     LLEnvManagerNew::getInstance()->useWaterParams(legacySettings);
    // }
    // if (mCurrentEnvironment->getDayCycle()) {
    //     legacySettings = LLSettingsVODay::convertToLegacy(mCurrentEnvironment->getDayCycle());
    //     LLEnvManagerNew::getInstance()->useDayCycleParams(legacySettings,LLEnvKey::SCOPE_LOCAL);
       
    // }
    //LLWLParamManager::getInstance()->update(cam);
...
void LLEnvironment::recordEnvironment(S32 parcel_id, LLEnvironment::EnvironmentInfo::ptr_t envinfo, LLSettingsBase::Seconds transition)
{
    ...
    // {
    //     LL_INFOS("ENVIRONMENT") << "Requested environment region id: " << envinfo->mRegionId << " agent is on: " << gAgent.getRegion()->getRegionID() << LL_ENDL;
    //     return;
    // }
...
{
    LL_INFOS() << "LLEnvironment::requestRegion " <<LL_ENDL;
...
{
    LL_INFOS() << "LLEnvironment::requestParcel " << parcel_id <<LL_ENDL;
    if (!isExtendedEnvironmentEnabled())
    ...
    coroRequestEnvironment(parcel_id,cb);
}
...
extern AIHTTPTimeoutPolicy environmentEepRequestResponder_timeout;
class LLEepEnvironmentRequestResponder: public LLHTTPClient::ResponderWithResult
{
	LOG_CLASS(LLEepEnvironmentRequestResponder);
public:
    LLEepEnvironmentRequestResponder(S32 parcel_id,LLEnvironment::environment_apply_fn apply);
	/*virtual*/ void httpSuccess(void);
	/*virtual*/ void httpFailure(void);
	/*virtual*/ AIHTTPTimeoutPolicy const& getHTTPTimeoutPolicy(void) const { return environmentEepRequestResponder_timeout; }  
	/*virtual*/ char const* getName(void) const { return "LLEepEnvironmentRequestResponder"; }

private:
	private:
    LLEnvironment::environment_apply_fn mApply;
	S32 mParcel_id;
    LLEnvironment::EnvironmentInfo::ptr_t envinfo;
	
};
LLEepEnvironmentRequestResponder::LLEepEnvironmentRequestResponder(S32 parcel_id,LLEnvironment::environment_apply_fn apply)
    :	mParcel_id(parcel_id),
        mApply(apply)
	{ } 
void LLEepEnvironmentRequestResponder::httpSuccess(void)
{
    LL_INFOS () << "coroRequestenvironment result " << mContent << LL_ENDL;
    LLSD environment = mContent["environment"];
    if (LLApp::isExiting())
    {
        return;
    }
    ...
    if (environment.isDefined() && mApply)
    {
        ...
        mApply(mParcel_id, envinfo);
        
    }
}

void LLEepEnvironmentRequestResponder::httpFailure(void)
{
...
void LLEnvironment::coroRequestEnvironment(S32 parcel_id, LLEnvironment::environment_apply_fn apply)
{
    ...
    LL_INFOS("ENVIRONMENT") << "Requesting for parcel_id=" << parcel_id << LL_ENDL;
    ...
    LLHTTPClient::get(url, new LLEepEnvironmentRequestResponder(parcel_id,apply));
    ...
    if (gSavedSettings.getBOOL("EnvironmentPersistAcrossLogin"))
    {
        ...
        if (environment)
        {
            ...
            if (day)
            {
                ...
                else if (!name.empty() && name != "_default_")
                {
