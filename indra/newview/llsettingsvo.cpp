...
void LLSettingsVOBase::createNewInventoryItem(LLSettingsType::type_e stype, const LLUUID &parent_id, inventory_result_fn callback)
{
    ...
    // create_inventory_item(gAgent.getID(), gAgent.getSessionID(), parent_id, LLTransactionID::tnull,
    //     LLSettingsType::getDefaultName(stype), "", LLAssetType::AT_SETTINGS, LLInventoryType::IT_SETTINGS, 
    //     stype, nextOwnerPerm, cb);    
    create_inventory_settings(gAgent.getID(), parent_id,LLTransactionID::tnull, cb);
}
...
void LLSettingsVOBase::createInventoryItem(const LLSettingsBase::ptr_t &settings, U32 next_owner_perm, const LLUUID &parent_id, std::string settings_name, inventory_result_fn callback)
{
    ...
    create_inventory_settings(gAgent.getID(), parent_id,tid, cb);
}
...
void LLSettingsVOBase::getSettingsAsset(const LLUUID &assetId, LLSettingsVOBase::asset_download_fn callback)
{
    ...
    gAssetStorage->getAssetData(assetId, LLAssetType::AT_SETTINGS,
								 [callback](LLVFS *vfs,const LLUUID &asset_id, LLAssetType::EType type, void *, S32 status, LLExtStat ext_status) 
             { onAssetDownloadComplete(vfs,asset_id, type,status, ext_status, callback); }, nullptr,
								 true);    

}

void LLSettingsVOBase::onAssetDownloadComplete(LLVFS *vfs,const LLUUID &asset_id, LLAssetType::EType type,S32 status, LLExtStat ext_status, LLSettingsVOBase::asset_download_fn callback)
{
    ...
    if (!status)
    {
		...
		LL_INFOS() << "EEP settings" << settings << LL_ENDL;
		if (!settings)
		{
			status = 1;
			LL_WARNS() << "Unable to create settings object for asset: "
					<< asset_id << LL_ENDL;
		}
		else
		{
...
void LLSettingsVOWater::applySpecial(void *ptarget, bool force)
{
    ...
    if (force || (shader->mShaderGroup == LLGLSLShader::SG_WATER))
	{
        ...
        // LLVector3 norm(0.f, 0.f, 1.f);
        // LLVector3 p(0.f, 0.f, water_height + 0.1f);
        ...
        //Genesis comment : need to import glh libraries in the project
    }
}
