...
public:
        ...
        S32& discard_level)
    {
		...
		if (num_threads>1) {
			...
			opj_codec_set_threads(decoder, num_threads);
		}
...
public:
    ...
    bool encode(const LLImageRaw& rawImageIn, LLImageJ2C &compressedImageOut)
    {
        ...
        if( parameters.irreversible ) {
            ...
            nb_layers = llclamp((int)nb_layers, 1, 6);
            ...
            max_cs_size = llmax((int)max_cs_size, (int)FIRST_PACKET_SIZE);
		...
		if (num_threads>1) {
			opj_codec_set_threads(encoder, num_threads);
		}
...
const char* fallbackEngineInfoLLImageJ2CImpl()
{
    ...
    return (std::string("OpenJPEG: " OPENJPEG_VERSION ", Runtime: ")
        + opj_version()).c_str();
#else
    return (std::string("OpenJPEG runtime: ") + opj_version()).c_str();
#endif
}