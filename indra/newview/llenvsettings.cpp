										 ...
										 asset_download_fn callback)
{
	...
	gAssetStorage->getAssetData(asset_id, LLAssetType::AT_SETTINGS,
								 LLEnvSettingsBase::onAssetDownloadComplete, (void*)storage,
								 true);
}
//static
void LLEnvSettingsBase::onAssetDownloadComplete(LLVFS *vfs,
                                const LLUUID& uuid,
                                ...
                                LLExtStat ext_status)
{
	...
	if (!status)
	{
		...
		{
			settings = LLEnvSettingsBase::createFromLLSD(llsdsettings);
		}
...
void LLEnvSettingsWater::applySpecial(LLShaderUniforms* targetp)
{
	...
	bool underwater = LLViewerCamera::instance().getOrigin().mV[2] - water_height <= 0.f;
